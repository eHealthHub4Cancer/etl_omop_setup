...
/************************************************/
DROP TABLE IF EXISTS @resultSchema.achilles_result_concept_count;

CREATE TABLE @resultSchema.achilles_result_concept_count
(
  concept_id                int,
  record_count              bigint,
...
WITH counts  AS (
  ...
  FROM @resultSchema.achilles_results
...
WITH concepts  AS (
  ...
  from (
    ...
    from tmp_counts c
    join @vocabDatabaseSchema.concept_ancestor ca on cast(ca.descendant_concept_id as varchar(50)) = c.concept_id
  ) c
  left join @vocabDatabaseSchema.concept_ancestor ca on c.concept_id = cast(ca.ancestor_concept_id as varchar(50))
)
 SELECT
ancestor_id,
  descendant_id
...
INSERT INTO @resultSchema.achilles_result_concept_count (concept_id, record_count, descendant_record_count, person_count, descendant_person_count)
SELECT DISTINCT
